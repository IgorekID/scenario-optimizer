...
class ScenarioApp:
    ...
    def __init__(self, root):
        ...
        tk.Label(root, text="Сценарии реагирования",
                 font=("Arial", 14)).pack()
        ...
        tk.Button(frame_buttons, text="Добавить",
                  command=self.add_scenario).pack(side='left', padx=5)

        tk.Button(frame_buttons, text="Удалить",
                  command=self.delete_scenario).pack(side='left', padx=5)

        tk.Button(frame_buttons, text="Загрузить шаблон",
                  command=self.load_template).pack(side='left', padx=5)

        tk.Button(frame_buttons, text="Загрузить Excel",
                  command=self.load_from_excel).pack(side='left', padx=5)

        # ===== ОГРАНИЧЕНИЯ =====

        tk.Label(root, text="Ограничения ресурсов",
                 font=("Arial", 12)).pack()
        ...
        tk.Button(frame_export, text="Экспорт Excel",
                  command=self.export_excel).pack(side='left', padx=10)
        ...
        tk.Button(frame_export, text="Экспорт PDF",
                  command=self.export_pdf).pack(side='left', padx=10)
    ...
    def load_template(self):
        ...
        data = [
            {'Name': 'Активация резервной системы', 'Cost': 900000, 'Time': 4, 'Personnel': 3, 'Risk': 0.8, 'Complexity': 0.6, 'Reliability': 0.9},
            {'Name': 'Анализ угрозы', 'Cost': 500000, 'Time': 8, 'Personnel': 2, 'Risk': 0.5, 'Complexity': 0.3, 'Reliability': 0.6}
        ]
...
"""
    ...
    def plot_radar(self, df):
        ...
        categories = ['Risk', 'Cost', 'Time', 'Complexity', 'Reliability']
        ...
        ax.set_xticklabels(['Risk', 'Cost', 'Time', 'Complexity', 'Reliability'])

        # место для легенды справа
